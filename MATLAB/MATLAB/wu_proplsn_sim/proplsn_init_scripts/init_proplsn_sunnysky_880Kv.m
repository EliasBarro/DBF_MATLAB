function init_proplsn_sunnysky_880Kv
% init function for the standalone propulsion model - populate parameters for
% Motor: https://sunnyskyusa.com/collections/x-motors/products/sunnysky-x3525-brushless-motors-520kv-880kv?variant=45197935887
% Max. continuous power: 1300 W

% %nimh battery tlu
% num_series                = 13; %0.0672 kg/cell
% num_parallel			  = 2;
...
path = 'UIUCpropDB/';
file = 'apce_14x12_kt1069_3507.txt'; %Pusher_tv
prop_table = importdata([path,file]);
prop_table = prop_table.data;
proplsn.prop_dia_in = 14;
propeller_mass = 0.0771;
n_blades = 2;
proplsn.num_props = 1;

% motor params for Sunnysky 3525 880Kv
proplsn.R_sys_ohm = 0.01 + proplsn.internal_resistance + (0.0047*0.5); %Ohm (resistance of the motor plus resistance of the battery plus wiring resistance)
proplsn.K_v = 880;      % Kv, rpm/volt
proplsn.K_T_Nm_A = 0.05; % Kt, Nm/A <---this was a guess that was iterated on until the efficiency look reasonable
I_0 = 2.5;              % No-load current A   % ^Basically, make sure that mechanical power is somewhere around 0.8-0.9 times electrical power
...
proplsn.dt      = 0.005; %The simulation timestep. Should be smaller than 0.01 (100 hertz) if oscillations
