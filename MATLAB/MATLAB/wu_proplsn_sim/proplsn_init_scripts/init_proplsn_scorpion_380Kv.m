function init_proplsn_scorpion_380Kv
% init function for the standalone propulsion model - populate parameters for
% Motor: https://www.scorpionsystem.com/catalog/aeroplane/motors_1/sii-40/SII-4035-380/
% Max. continuous power:	2600W
% Max. continuous current:	70A
% Mass:						435g

%nimh battery tlu
% % num_series                = 23; %5000mAh: 0.0672 kg/cell   2100mAh: 0.0329 kg/cell
% % num_parallel			  = 2;
...
% cell_max_cap_Ah = 2.1;
% cell_internal_resistance = 0.014;
...
path = 'UIUCpropDB/';
file = '15x10at8700rpmQProp.txt'; %Propeller: https://www.apcprop.com/product/13x8/
prop_table = importdata([path,file]);
prop_table = prop_table.data;
proplsn.prop_dia_in = 15; %Scales prop: can tweak this by small amounts (1" for >12" dia.; 0.5" for <12" dia. props)
propeller_mass = 0.0771;
n_blades = 2;
proplsn.num_props = 1;

% motor params for Scorpion 380Kv
proplsn.R_sys_ohm = 0.025 + proplsn.internal_resistance + (0.0047*0.5); %Ohm (resistance of the motor plus resistance of the battery plus wiring resistance)
proplsn.K_v = 380;      % Kv, rpm/volt
proplsn.K_T_Nm_A = 0.18; % Kt, Nm/A <---this was a guess that was iterated on until the efficiency look reasonable
I_0 = 1.52;              % No-load current A   % ^Basically, make sure that mechanical power is somewhere around 0.9 times electrical power
