%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% motor_model_hyperion_testbed
% Author: Austin Stover
% Date: October-November 2018
% Analytic simulation of Hyperion and design derivatives
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clear all;

%TODO: Make plot of efficiency over Kv, velocity (for z-axis). Slider for
%pitch using J gain. Same for v required.

%Goal: Maximize dynamic thrust for some battery mass and flight time
batt_Mass = 1.3;
t_min = 10; %M2 time for hyp1_1: 120.4008/60 m
num_series = 6; %12;
v_nom = 3.7;

lipo_Spec_E_Wh_kg = 150;
E_tot_Wh = batt_Mass*lipo_Spec_E_Wh_kg;
Cap = E_tot_Wh/(num_series*v_nom)
P_mean = E_tot_Wh/(t_min/60)
i_mean = P_mean/(num_series*v_nom)

stat_T_des_kgf = 10; %Desired static thrust
% Aero Params
V = 30; %Estimated velocity (m/s) for dynamic thrust
rho = 1.16; %Air density (kg/m^3)

%Propulsion Params
K_v_list = 5:10:2500; %Motor Kv (RPM/V) Hyperion used 1300
I_0 = 0.5; %Motor no-load current (A)
...
%Dynamic prop data
dyn_file = 'apce_9x6_rd0994_6715.txt';%'apce_9x7.5_2551rd_6014.txt';%'apce_10x7_pg0817_6531.txt';%'apce_8x8_2813rd_6418.txt'; %'15x10at8700rpmQProp.txt'; %'rusp_11x4_static_2952os.txt';
dyn_prop_table = importdata([path,dyn_file]);
dyn_prop_table = dyn_prop_table.data;

%Static prop data
stat_file = 'apce_9x6_static_rd0987.txt';%'apce_9x7.5_static_2545rd.txt';%'apce_10x7_static_pg0811.txt';%'apce_8x8_static_2805rd.txt'; %15x10_static_QProp.txt'; %'rusp_11x4_static_2952os.txt';
stat_prop_table = importdata([path,stat_file]);
stat_prop_table = stat_prop_table.data;

prop_dia_in = 9; %Prop Diameter
n_blades = 2;
...
% figure;
% plot(dyn_J,dyn_CT,dyn_J,dyn_CP);
% xlabel('J');
% legend('C_T','C_P');

figure;
RPM = V./(dyn_J.*D).*(60/(2*pi));
plot(RPM,dyn_CT,RPM,dyn_CP);
xlabel('RPM');
legend('C_T','C_P');
